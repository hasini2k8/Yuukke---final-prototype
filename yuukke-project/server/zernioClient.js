...
// instead of calling Instagram/LinkedIn's own APIs directly so non-technical
// sellers can connect their accounts through one simple hosted login page
// instead of generating developer-console tokens themselves. Zernio already
// holds the platform approvals that would otherwise require Yuukke to
// register its own app with each network and go through review. Free for
// the first 2 connected accounts.
...
// One profile per seller — created once, the first time they connect
// anything. Keyed by Zernio's own opaque profile id, not by our seller id.
...
// per platform (instagram, linkedin).
...
// scheduledFor/scheduledTime are a naive local date (YYYY-MM-DD) + time
// (HH:mm) in IST — Zernio's own infrastructure fires the post later, so our
// server doesn't need to be running at that moment.
export async function createPost({ platform, accountId, caption, mediaPublicUrl, scheduledFor, scheduledTime }) {
  const body = {
    content: caption,
  ...
  if (scheduledFor) {
    body.scheduledFor = `${scheduledFor}T${scheduledTime || "09:00"}:00`;
