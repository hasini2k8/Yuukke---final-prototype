...
const STRINGS = [
  "Explore Marketplace", "ABOUT YUUKKE", "Talent is everywhere. Opportunity isn't.",
  ...
  "Five pillars of enablement", "Join the movement", "Buy, mentor, learn, or sell — there's a way in for everyone.",
  ...
  return (
    <div style={{ background: theme.cream, minHeight: "100vh" }}>
      ...
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "60px 24px 80px" }}>
        <p style={{ fontSize: 12, fontWeight: 700, color: theme.wine, letterSpacing: 1.5, marginBottom: 10 }}>{t("ABOUT YUUKKE")}</p>
        <h1 style={{ fontFamily: theme.fontDisplay, fontSize: 36, color: theme.ink, margin: "0 0 20px", lineHeight: 1.25 }}>
          {t("Talent is everywhere. Opportunity isn't.")}
        </h1>
        <p style={{ fontSize: 15, color: theme.inkSoft, lineHeight: 1.7, marginBottom: 40 }}>
          {t("Yuukke exists because skill without market access stays invisible. We're a marketplace and enablement platform for women entrepreneurs — bridging the gap between real talent and the visibility, tools, and income it deserves.")}
        </p>

        <h2 style={{ fontFamily: theme.fontDisplay, fontSize: 22, color: theme.ink, margin: "0 0 28px" }}>{t("Five pillars of enablement")}</h2>
        <div style={{ display: "flex", flexDirection: "column", gap: 24, marginBottom: 50 }}>
          {PILLARS.map((p) => (
            <div key={p.n} style={{ display: "flex", gap: 18, alignItems: "flex-start" }}>
              ...
              <div>
                <p style={{ fontSize: 14.5, fontWeight: 700, color: theme.ink, margin: "0 0 4px" }}>{t(p.title)}</p>
                <p style={{ fontSize: 13, color: theme.inkSoft, margin: 0, lineHeight: 1.6 }}>{t(p.desc)}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ background: theme.wineTint, borderRadius: 18, padding: 30, textAlign: "center" }}>
          <p style={{ fontFamily: theme.fontDisplay, fontSize: 19, color: theme.ink, margin: "0 0 8px" }}>{t("Join the movement")}</p>
          <p style={{ fontSize: 13.5, color: "#5a3f47", marginBottom: 20 }}>{t("Buy, mentor, learn, or sell — there's a way in for everyone.")}</p>
