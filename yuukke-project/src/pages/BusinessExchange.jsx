...
const STRINGS = [
  "Explore Marketplace", "BUSINESS EXCHANGE", "Connect your business with builders who deliver.",
  ...
  "Register your interest", "Thanks,", "— our partnerships team will reach out.",
  ...BENEFITS.flatMap((b) => [b.title, b.desc]),
];
  ...
  return (
    <div style={{ background: theme.cream, minHeight: "100vh" }}>
      ...
      <div style={{ maxWidth: 700, margin: "0 auto", padding: "60px 24px 80px", textAlign: "center" }}>
        <p style={{ fontSize: 12, fontWeight: 700, color: theme.wine, letterSpacing: 1.5, marginBottom: 10 }}>{t("BUSINESS EXCHANGE")}</p>
        <h1 style={{ fontFamily: theme.fontDisplay, fontSize: 32, color: theme.ink, margin: "0 0 16px" }}>
          {t("Connect your business with builders who deliver.")}
        </h1>
        <p style={{ fontSize: 14.5, color: theme.inkSoft, lineHeight: 1.7, marginBottom: 40, maxWidth: 520, marginLeft: "auto", marginRight: "auto" }}>
          {t("Whether you're sourcing corporate gifts, looking for vendors, or want to bring women-led products into your own supply chain — the Business Exchange is where organizations and builders meet.")}
        </p>
          ...
          {BENEFITS.map((b) => {
            ...
            return (
              <div key={b.title} style={{ background: theme.white, border: `1px solid ${theme.line}`, borderRadius: 16, padding: 22 }}>
                ...
                <p style={{ fontFamily: theme.fontDisplay, fontSize: 15, color: theme.ink, margin: "0 0 6px" }}>{t(b.title)}</p>
                <p style={{ fontSize: 12.5, color: theme.inkSoft, margin: 0, lineHeight: 1.6 }}>{t(b.desc)}</p>
              </div>
            );
          })}
        </div>
        ...
        {user ? (
          <p style={{ fontSize: 14, color: theme.ink, fontWeight: 600 }}>{t("Thanks,")} {user.businessName || user.email} {t("— our partnerships team will reach out.")}</p>
        ) : (
          <button onClick={() => setShowLogin(true)} style={{
            ...
            {t("Register your interest")} <ArrowRight size={14} />
          </button>
        )}
      </div>
