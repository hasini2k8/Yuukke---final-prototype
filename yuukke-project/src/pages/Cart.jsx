...
const STRINGS = [
  "Continue shopping", "Your cart", "Log in from the marketplace to see your cart.", "Loading your cart…",
  "Your cart is empty", "Browse the marketplace to find something made with care.", "Browse marketplace",
  "each", "Total", "Proceed to checkout",
];
  ...
  return (
    <div style={{ background: theme.cream, minHeight: "100vh" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 40px", borderBottom: `1px solid ${theme.line}`, background: theme.white }}>
        ...
        <a href="/marketplace" style={{ fontSize: 13.5, fontWeight: 600, color: theme.wine, textDecoration: "none" }}>{t("Continue shopping")}</a>
      </div>

      <div style={{ maxWidth: 760, margin: "0 auto", padding: "40px 24px 90px" }}>
        <h1 style={{ fontFamily: theme.fontDisplay, fontSize: 28, color: theme.ink, margin: "0 0 24px" }}>{t("Your cart")}</h1>

        {!authLoading && !user && (
          <p style={{ fontSize: 14, color: theme.inkSoft }}>{t("Log in from the marketplace to see your cart.")}</p>
        )}

        {user && loading && (
          <div style={{ display: "flex", alignItems: "center", gap: 8, color: theme.inkSoft }}>
            <Spinner size={16} color={theme.wine} /> {t("Loading your cart…")}
          </div>
        )}
        ...
        {user && !loading && cart.items.length === 0 && (
          <div style={{ textAlign: "center", padding: "60px 20px", background: theme.white, borderRadius: 18, border: `1px solid ${theme.line}` }}>
            ...
            <p style={{ fontFamily: theme.fontDisplay, fontSize: 18, color: theme.ink, margin: "0 0 6px" }}>{t("Your cart is empty")}</p>
            <p style={{ fontSize: 13.5, color: theme.inkSoft, margin: "0 0 18px" }}>{t("Browse the marketplace to find something made with care.")}</p>
        ...
        {user && !loading && cart.items.length > 0 && (
          <>
              ...
              {cart.items.map((item, i) => (
                <div key={item.productId} style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ fontSize: 12, color: theme.inkSoft, margin: 0 }}>₹{Number(item.product.price).toLocaleString("en-IN")} {t("each")}</p>
                  </div>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
              <span style={{ fontSize: 15, fontWeight: 700, color: theme.ink }}>{t("Total")}</span>
            ...
            <button onClick={() => navigate("/checkout")} style={{
              ...
              {t("Proceed to checkout")} <ArrowRight size={15} />
            </button>
          </>
        )}
      </div>
