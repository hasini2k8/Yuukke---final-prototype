...
Generated automatically above — Yuukke's AI names your business, writes its story, and designs a logo. Publish it to get a real page your customers can visit.
            ...
            <details open style={{ marginBottom: 20, border: `1px solid ${theme.line}`, borderRadius: 13, padding: 14, background: theme.cream }}>
              <summary style={{ cursor: "pointer", fontSize: 12.5, fontWeight: 800, color: theme.ink }}>Edit website content</summary>
              <p style={{ fontSize: 11, color: theme.inkSoft, lineHeight: 1.5 }}>Fine-tune the website here, or describe broader changes to the website assistant above. To add a customer chatbot, ask the assistant to create one.</p>
              {[["announcement", "Announcement bar", "e.g. Free delivery this festive week"], ["heroHeadline", "Hero headline", "Main website heading"], ["heroSubheadline", "Hero description", "Short reason customers should explore"], ["primaryCTA", "Primary button", "e.g. Shop the collection"], ["secondaryCTA", "Secondary button", "e.g. Read our story"], ["featuredTitle", "Product section heading", "e.g. Customer favourites"], ["mission", "Business mission", "Why this business exists"], ["story", "Our story", "The entrepreneur's story"], ["newsletterHeading", "Newsletter heading", "Stay in the loop"], ["newsletterText", "Newsletter description", "What subscribers will receive"]].map(([field, label, placeholder]) => <label key={field} style={{ display: "block", marginTop: 10 }}><span style={{ display: "block", fontSize: 10.5, fontWeight: 800, color: theme.ink, marginBottom: 5 }}>{label}</span>{["story", "mission", "heroSubheadline", "newsletterText"].includes(field) ? <textarea rows={2} value={config.websiteContent?.[field] || ""} onChange={(e) => updateWebsiteContent(field, e.target.value)} placeholder={placeholder} style={{ width: "100%", padding: "9px 10px", border: `1px solid ${theme.line}`, borderRadius: 9, background: theme.white, fontFamily: theme.fontBody, resize: "vertical" }} /> : <input value={config.websiteContent?.[field] || ""} onChange={(e) => updateWebsiteContent(field, e.target.value)} placeholder={placeholder} style={{ width: "100%", padding: "9px 10px", border: `1px solid ${theme.line}`, borderRadius: 9, background: theme.white, fontFamily: theme.fontBody }} />}</label>)}
              ...
              <p style={{ fontSize: 10.5, fontWeight: 800, color: theme.ink, margin: "14px 0 6px" }}>Shop policies</p>
              <div style={{ display: "grid", gap: 8 }}>{[["shipping", "Shipping information"], ["returns", "Returns information"], ["customOrders", "Custom-order information"]].map(([field, label]) => <label key={field}><span style={{ display: "block", fontSize: 10.5, fontWeight: 800, marginBottom: 4 }}>{label}</span><textarea rows={2} value={config.websiteContent?.policies?.[field] || ""} onChange={(e) => updateWebsiteContent("policies", { ...(config.websiteContent?.policies || {}), [field]: e.target.value })} style={{ width: "100%", padding: "8px", border: `1px solid ${theme.line}`, borderRadius: 8, fontFamily: theme.fontBody, resize: "vertical" }} /></label>)}</div>
            </details>
