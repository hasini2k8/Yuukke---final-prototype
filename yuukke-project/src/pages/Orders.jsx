...
const STRINGS = [
  "Back to marketplace", "My orders", "Log in from the marketplace to see your orders.", "Loading your orders…",
  "No orders yet", "Orders you place will show up here.", "Total",
];
  ...
  return (
    <div style={{ background: theme.cream, minHeight: "100vh" }}>
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "40px 24px 90px" }}>
        <h1 style={{ fontFamily: theme.fontDisplay, fontSize: 28, color: theme.ink, margin: "0 0 24px" }}>{t("My orders")}</h1>

        {!authLoading && !user && <p style={{ fontSize: 14, color: theme.inkSoft }}>{t("Log in from the marketplace to see your orders.")}</p>}

        {user && loading && (
          <div style={{ display: "flex", alignItems: "center", gap: 8, color: theme.inkSoft }}>
            <Spinner size={16} color={theme.wine} /> {t("Loading your orders…")}
          </div>
        )}
        ...
        {user && !loading && orders.length === 0 && (
          <div style={{ textAlign: "center", padding: "60px 20px", background: theme.white, borderRadius: 18, border: `1px solid ${theme.line}` }}>
            ...
            <p style={{ fontFamily: theme.fontDisplay, fontSize: 18, color: theme.ink, margin: "0 0 6px" }}>{t("No orders yet")}</p>
            <p style={{ fontSize: 13.5, color: theme.inkSoft, margin: 0 }}>{t("Orders you place will show up here.")}</p>
          </div>
        )}
